...
Now, let's edit the RHSM username. To do that, open the script and change according to below
...
As you can see below, there are some options. `sync_data`, `process_file` and `remove_uuid_list` but BEFORE will be necessary we run the command `"took token -e rhsm-auth <rhsm_username>"` to start the session.
```
$ ./rhsm_api.sh 
...
At this time we will use the flag `sync_data`. This call will be responsible for download all the information about Content Hosts registered on the customer portal. According to the number of registered Content Hosts, this process can spend some time.
```
$ ./rhsm_api.sh sync_data
...
Done
```
Note. The script will finish when the `Status == 0`.
...
Awesome, at this time, we can check the list on the file `/tmp/rhsm_report.log`.
```
...
```
Note. here you will see a complete list, ordered by lastcheckin, so the initial lines means the Content Host updated recently and at the bottom you will see all older machines and/or without lastcheckin.

To our example, I would like to remove all machines without `lastcheckin`. To do that, let's create a new file `/tmp/remove_uuid` with the content as below:
```
localhost.localdomain,6d45a9da-222f-4c25-b919-8f13e2397a96,Virtual,,
base,03e15f8f-511e-401c-aea5-a03fcb9011df,Physical,,
```
...
Special thanks to shane "Shane McDowell" and peasters "Patrick Easters"